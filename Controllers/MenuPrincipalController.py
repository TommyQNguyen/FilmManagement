...
from Controllers.MenuGererLesCategoriesController import (
    MenuGererLesCategoriesController,
)
...
class MenuPrincipalController:
    ...
    def navigate(self, menuChoice):
        if menuChoice == "1":
            MenuCreationFilmController(getBackToMenuPrincipal=self.init).init()
        if menuChoice == "2":
            MenuAfficherLesFilmsController(getBackToMenuPrincipal=self.init).init()
        if menuChoice == "3":
            MenuRechercherUnFilmController(getBackToMenuPrincipal=self.init).init()
        if menuChoice == "4":
            MenuGererLesCategoriesController(getBackToMenuPrincipal=self.init).init()
        if menuChoice == "5":
            MenuExporterFilmController(getBackToMenuPrincipal=self.init).init()
